...
def update_stack_resources(credentials, template, stackname, stackregion, existing_accountid):
    datestamp = time.strftime("%d/%m/%Y")
    client = boto3.client('cloudformation',
                          aws_access_key_id=credentials['AccessKeyId'],
                          aws_secret_access_key=credentials['SecretAccessKey'],
                          aws_session_token=credentials['SessionToken'],
                          region_name=stackregion)
    print("Updating stack " + stackname + " in " + existing_accountid)
    time.sleep(60)
    updating_stack = True
    while updating_stack is True:
        try:
            updating_stack = False
    ...
    stack_building = True
    print("Stack updation in process...")
...
def main(event,context):
    ...
    if(event['RequestType'] == 'Update'):
        ...
        for o in outputs:
            if o['OutputKey'] == "AccountID":
                existing_accountid = o['OutputValue']
                print("Accountid: ",existing_accountid)
                try: 
                    credentials = assume_role(existing_accountid, accountrole)
                    template = get_template(sourcebucket,baselinetemplate)
                    cf_update_stack = update_stack_resources(credentials, template, stackname, stackregion, existing_accountid)
                    print(cf_update_stack)
                    print("CF Stack Update for account " + existing_accountid +" complete!")
                    respond_cloudformation(event, "SUCCESS", { "Message": "Resource update successful!","AccountID" : existing_accountid, "LoginURL" : "https://"+existing_accountid+".signin.aws.amazon.com/console", "Username" : ServiceCatalogUserName })
